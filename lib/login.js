...
const states = [
    ...
    },
    {
        ...
        async handler(page, selected) {
            ...
            // eslint-disable-next-line
            (description) => description.textContent, selected);
    ...
    },
    {
        ...
        async handler(page, _selected, noPrompt, _defaultUsername, _defaultPassword, defaultTfaSecret) {
            const error = await page.$(".alert-error");
            if (error) {
                debug("Found error message. Displaying");
                // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
            ...
            if (noPrompt && defaultTfaSecret) {
                ...
                const generatedCode = otplib_1.authenticator.generate(defaultTfaSecret);
                if (generatedCode) {
                    ...
                    ({ verificationCode } = await inquirer_1.default.prompt([
                        {
    ...
    },
    {
        ...
        async handler(page, _selected, _noPrompt, _defaultUsername, _defaultPassword, _defaultTfaSecret, rememberMe) {
            if (rememberMe) {
...
exports.login = {
    async loginAsync(profileName, mode, disableSandbox, noPrompt, enableChromeNetworkService, awsNoVerifySsl, enableChromeSeamlessSso, noDisableExtensions) {
        ...
        const samlResponse = await this._performLoginAsync(loginUrl, headless, disableSandbox, cliProxy, noPrompt, enableChromeNetworkService, profile.azure_default_username || "", profile.azure_default_password, profile.azure_default_tfa_secret, enableChromeSeamlessSso, profile.azure_default_remember_me || false, noDisableExtensions);
        const roles = this._parseRolesFromSamlResponse(samlResponse);
        const { role, durationHours } = await this._askUserForRoleAndDurationAsync(roles, noPrompt, profile.azure_default_role_arn || "", profile.azure_default_duration_hours || "1");
    ...
    async _loadProfileAsync(profileName) {
        let profile = await awsConfig_1.awsConfig.getProfileConfigAsync(profileName);
     ...
     * @private
     */
    async _performLoginAsync(url, headless, disableSandbox, cliProxy, noPrompt, enableChromeNetworkService, defaultUsername, defaultPassword, defaultTfaSecret, enableChromeSeamlessSso, rememberMe, noDisableExtensions) {
                ...
                while (true) {
                    ...
                    for (let i = 0; i < states.length; i++) {
                        ...
                        if (selected) {
                            ...
                            await Promise.race([
                                samlResponsePromise,
                                state.handler(page, selected, noPrompt, defaultUsername, defaultPassword, defaultTfaSecret, rememberMe),
                            ]);
